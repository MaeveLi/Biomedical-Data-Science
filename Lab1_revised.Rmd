...
### Exercise 1.2
Assign the value of "3 + 5" to the object "an.object" using the assignment operator "<-", by typing "an.object <- 3 + 5", and press enter. Now look under the Environment tab.
Create another object called "new.object" and assign the value "an.object". What is the numeric value of "new.object"?
Next, assign the value "3" to "new.object" and look under the Environment tab. 

You should observe that the value of "new.object" has been overwritten but "an.object" is unchanged. In R, the assignment operator merely assigns the value of an object to another, it does not link them. 
...
R Markdown files can combine text with executable code. You can embed an R code chunk like this:
...
When you click the **Knit** button, a document will be generated that includes both content as well as the output of any embedded R code chunks within the document. This is very useful for generating reports and you will be expected to submit both assessments as R Markdown files. 

For now, close the document you generated using "Knit" and return to the IDE. You may notice that the Console pane has been minimised. This is a useful feature as you are now running R in interactive mode from within the R Markdown file and you no longer need to view the Console pane. Note that the code still runs through the console. 


## 2.2 Notes regarding input

R commands are case sensitive. If you get a function not found error, check spelling and capitalization.

The # symbol mutes R code. Run the following chunk with each line muted in turn. 
...
Spaces have no meaning in R and can be used to make your input easier to read but shouldn't be entered in the middle of an object name.
...
Similarly, you might want to split long commands over several lines to make it easier to read. R doesn't have a special character to continue reading the next line but it will do so if it considers that the command is "incomplete". A simple way to make sure that all the required lines are read is to surround your command with brackets. R won't stop reading until all the brackets are closed.
...
You can call a group of R commands by using a function. The basic form of a function is "function.name(arguments, options)". 
Even if there are no arguments or options, the parentheses are still mandatory. If you leave off the parentheses, R will print out the source code for the function you have entered. 
...
Make a note about anything you find unclear in the user manual format, we will start the lab by checking any questions you have regarding the material up to this point and the user manual in particular.
...
If you want to use a package that isn't listed you must install it first. You can do this by entering the install.packages("packageName") command or selecting "Install" in the Packages tab. 
...
```{r}
sort(islands)[c(length(islands)-9,10)] ##注意用c才能输出多个位置
#or第二种方法
...
```{r }
bins <- seq(0, 4000, by=1000) #给出分界点
table(cut(rivers, bins))  #cut(数据集，分界点的seq vector),cut这个命令就已经把rivers数据集分类了
```
...
The breaks in the dataset are automatically computed by R and they offer a good starting point. We can
always force the breaks in the histogram to get a more tailored view of our dataset. Let’s also add better labels (options xlab and ylab) and title (option main) to the plot.
```{r}
bins <- seq(0, 4000, by=250)
hist(rivers, breaks=bins, #通过breaks人为划分组
...
Other less commonly used mathematical operators are %/% for integer division and %% for modulus (which
returns the remainder after integer division).
```{r}
7 %/% 2 #除数(integer division)
7 %% 2 #余数
```
...
```{r}
#islands3 <- which(islands%%3==0) ##which返回的是index！！！返回的不是值
...
Note that your numbers will be different as they depend on the seed of the random number generator. To make sure that you will get exactly the same sequence of random numbers, you need to set a seed explicitly.
This is very important for reproducibility of results.
```{r}
rnorm(5) 
rnorm(5)
```
...
### Exercise 4.2
Compute mean and standard deviation of the rivers vector, and use them to create a new vector called randomrivers (of the same length as rivers) of normally distributed points according
to those parameters (set the random seed to 1 beforehand). 
```{r}
mean_rivers <- mean(rivers)
sd_rivers <- sd(rivers)
randomrivers <- rnorm(length(rivers),mean=mean_rivers,sd=sd_rivers)
```

In reporting results, we generally don’t need more than 2-3 decimal places. This can be achieved using either signif() or round(): #round是保留小数位，signif是保留有效数字
```{r}
...
What can be said about the distribution of rivers? In what way could we investigate this? #这是一个单独的问题？和散点图无关

The distribution of river lengths from the rivers data set appears to be right-skewed. Plotting a histogram using the hist() function and/or examining descriptive statistics would give us a better idea of the distribution of the rivers data.
```{r}
...
### Exercise 4.7
Report names and sizes of islands, from the built-in islands dataset, with area in the first quartile, and compute the median area in
this subset.
```{r}
islands[which(islands <= quantile(islands,0.25))]
sub_islands <- islands[which(islands <= quantile(islands,0.25))]
median(sub_islands)
```
...
```{r}
hist(setdiff(1:max(rivers),rivers),
     main="Histogram of integers not in rivers vector",
     xlab="Integers")
```
...
round(mean(setdiff(islands3,rivers)),1) ##用setdiff相同数值的会被去掉
round(mean(islands3[!islands3 %in% rivers]), 1) ##而这种方法是根据index保留的，，可以保留每一个数值
```
```{r} 
#一个针对上述问题的simple example
vec1 <- c(1,2,2,3,3,4) 
vec2 <- c(1,4)
setdiff(vec1,vec2)
```
...
For more advanced matches or substitutions, you may need to learn about regular expressions, which are
ways of constructing rules to capture a specific pattern in a string. Here are some basic examples:
```{r}
...
Note that this doesn’t count the column headers and row names. To see the names of the columns (the
variable name) and of the rows use colnames() and rownames().
```{r}
colnames(USArrests)
rownames(USArrests)
```
...
Every entry in this table-like structure is indexed by two numbers, the row number and the column number.
You can also refer to the values in the dataframe by row name and column name (if these exist).
```{r}
USArrests[1, 1]
...
```{r}
a.rating <- attitude$rating
median(a.rating)
range(a.rating)
quantile(a.rating,c(0.25,0.75))
```


## 6.1 Subsetting

Dataframes can be subset using the [] operator the following way: dataframe.name[required rows/observations, required columns/variables]. 
We may want only a fraction of the observations but all columns: in this case, list which observations (by name or by index) are needed, and leave the column blank.
```{r}
...
A convenient function when dealing with operations on several columns of a dataframe is with(), which
allows to use column names directly without needing to use $.
```{r}
...
### Exercise 6.2
Report the median rating for observations that have above median values for the “raises” variable. (In the Attitude dataset)
```{r}
atti.subset <- attitude[attitude$raises>median(attitude$raises),]
median(atti.subset$rating)
#or
...
```{r}
par(mfrow=c(1,2))
hist(attitude$rating,main="Histogram of the rating in the attitude dataframe",
     xlab="rating")
boxplot(attitude$rating,main="Boxplot of the rating in the attitude dataframe",
        xlab="rating")
```
...
```{r}
twoplots <- function(varname,dataset){
  par(mfrow=c(1,2))
  
  hist(dataset[,varname],main=paste("Histogram of",deparse(substitute(varname)),"in",deparse(substitute(dataset))),
       xlab=deparse(substitute(variable)))     ##在dataset里面用变量选择某一列的时候要加引号，所以其实不需要deparse这一段命令，但为了记住这个命令就不删除了
  
  boxplot(dataset[,varname],main=paste("Boxplot of",deparse(substitute(varname)),"in",deparse(substitute(dataset))),
          xlab=deparse(substitute(variable)),ylab="value")
}
twoplots("rating",attitude)
sapply(colnames(attitude), twoplots, dataset = attitude) 
```
...
sbsplots <- function(varname, vars){
  par(mfrow=c(1,2))
  First <- varname %>% substr(1,1) %>% toupper()
  varname.lab <- varname %>% substr(2,nchar(.)) %>% paste0(First,.)
...
sapply(colnames(attitude), sbsplots, vars = attitude)  
```


# 7 Data Tables

The data.table is a high performance enhancement to the data.frame. As an extension of the dataframe, the data.table provides all the benefits, but with higher memory efficiency, extremely fast aggregation, updates and joins with a simplified syntax.
Data tables are defined with the data.table() function. Existing dataframes are compatible and can be converted by passing the dataframe to the data.table function. Likewise, the data table object may be converted back to a dataframe using the data.frame(). You can also convert existing unnamed list, data frame of data table by reference using setDT.
Data tables can be expressed with the notation: DT[i, j, by]. This is pronounced take DT, subset rows using i, then calculate j grouped by by. 
...
As datatables don’t support row names, it’s often sensible to keep the rownames as a column using the
keep.rownames option.
```{r}
mtcars.dt <- data.table(mtcars, keep.rownames=TRUE) # Defaults the rowname to "rn"
head(mtcars.dt)
```

The rn default name can be overridden by supplying the name you prefer.
```{r}
...
The list function may be used inside the data table to modify column content and column names. This is
often useful when merging tables where some column names may clash.
```{r}
...
Note that you can use a "." as an alias of the list command. It wasn't used earlier to avoid confusion while introducing the concept of piping.
```{r}
...
diabetes.dt[, .SD[1], by=.(dm.type, sex)]  ##SD:subset of datatable
```
...
Convert nottem to a data table format. 
Hint: Use the c() function to keep the most general class and convert a ts object to a numeric vector. 
```{r}
library(lubridate)
class(nottem) ##是时序(ts)class的object，用c来转换温度为最普遍的class(此类情况如上所述是转换为了numeric vector)，
##再用time把时序object的时间转换为小数(转换为小数是为了方便转换成datatable?)
#然后再用date_decimal把小数转换为年份和月份
nottem.dt <- nottem %>%
  data.table(temp = c(.),year = (c(time(.)))) %>%
  .[,list(
    month = format(
      date_decimal(year+.01), "%b"), # #abbreviated month
    Year = format(
      date_decimal(year+.01), "%Y"), #4-digit year
    temp)]
#格式：https://www.statmethods.net/input/dates.html
```

Use the reshape function (see R documentation) to reshape your data table to a "wide" format where the months are the variables with a temperature observation for each year.
Remove any additional strings so that the column names show only the months.
Using the assign by reference := assignment operator, add a column called summer.avg and add the mean temperature of the summer months June to September for each year.
Finally, using the assign by reference := assignment operator, add a column called june.decade.avg which provides the average temperature in June from 1920 to 1929 and 1930 to 1939.
```{r}
#原来的数据都是列的(相当于long?)
#用reshape把它变成wide的模式,months变成variable
nottem.wide <- reshape(nottem.dt, idvar = "Year", # idvar is the variable which need to be left unaltered
                      timevar = "month", # timevar are the variables that needs to converted to wide format
                      direction = "wide") # direction is, to which format the data needs to be transformed

# remove additional strings
colnames(nottem.wide) <- nottem.wide %>% 
                        colnames() %>% 
                        gsub("temp.", "", .) ##可以验证一下 打开以后的列名都是有temp的

nottem.wide <- nottem.wide %>%
                .[, summer.avg := rowMeans(.SD), ##SD是full row? rowmeans
                    by = .I, .SDcols = colnames(.[, Jun:Sep])] %>% ## I就是都搞 for each year 相当于不分组?
                .[, jun.decade.avg := mean(Jun), 
                    by=substr(Year, 0, 3)] ##只看前三位 把year分成了192和193
